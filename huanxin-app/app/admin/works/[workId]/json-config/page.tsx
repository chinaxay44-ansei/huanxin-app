...
export default function WorkJsonConfigPage() {
  ...
  useEffect(() => {
    ...
    const idx = parts.findIndex(p => p === 'works')
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch(`/api/admin/works/json-config/${wid}`, { headers: adminToken ? { 'x-admin-token': adminToken } : undefined })
        const json = await res.json()
        if (json?.config) setConfig(json.config as Config)
      } catch {}
    }
    if (mounted && wid) load()
  }, [mounted, wid, adminToken])

  const addNode = () => {
    setConfig((prev) => ({ ...prev, nodeInfoList: [...prev.nodeInfoList, { nodeId: '', fieldName: '', fieldValue: TOKENS.prompt_text, valueSource: 'prompt_text', description: '', defaultValue: '', visible: true, required: true, useUploadedMediaAsDefault: true }] }))
  }
  ...
  const save = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/works/json-config/${wid}`, {
        method: 'POST',
      ...
      toast.success('已保存')
      const re = await fetch(`/api/admin/works/json-config/${wid}`, { headers: adminToken ? { 'x-admin-token': adminToken } : undefined })
      const j2 = await re.json()
      if (j2?.config) setConfig(j2.config as Config)
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold">作品请求 JSON 配置</h1>
      ...
      <Card className="p-4 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2 flex flex-col gap-1 rounded-md border p-3 bg-muted/30">
            ...
            <p className="text-xs text-muted-foreground">开启后同款生成会携带 "instanceType": "plus" 参数</p>
          </div>
        </div>
      </Card>
      ...
      <Card className="p-4 space-y-4">
          ...
          {config.nodeInfoList.map((n, idx) => {
            ...
            return (
              <div key={idx} className="grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
                ...
                <div>
                  <label className="text-sm mb-1 block">说明（仅展示，不参与请求）</label>
        ...
        <div className="flex justify-end">
          <Button disabled={saving || !wid} onClick={save}>{saving ? '保存中…' : '保存配置'}</Button>
        </div>
      </Card>
    </div>
  )
