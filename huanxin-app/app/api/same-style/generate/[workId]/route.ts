...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (!workId) return Response.json({ success: false, message: '缺少作品ID' }, { status: 400 })
    ...
    const { data: work } = await supabase
      ...
      .eq('id', workId)
      .single()
    const reqJson = (work as any)?.generation_params?.request_json || {}
    ...
    if (!workflowId) {
      return Response.json({ success: false, message: '未配置 workflowId' }, { status: 400 })
    }

    // 将配置与客户端解析值合并为 nodeInfoList
    const TOKENS = {
    ...
    const nodeInfoList = nodeConfigs.map((nc, idx) => {
      ...
      if (isToken) {
        ...
        if (raw === TOKENS.PROMPT_TEXT) {
          fv = visible ? (typeof promptText === 'string' ? promptText : '') : ((nc as any)?.defaultValue ?? '')
    ...
    const uniqueTag = `${userId}-${taskId}-${Date.now()}`
    ...
    const request_json = {
      ...
      nodeInfoList: nodeConfigs.map((nc, idx) => {
        ...
        const visible = (nc as any)?.visible !== false
        let defVal = ''
        if (raw === TOKENS.PROMPT_TEXT) defVal = visible ? (typeof promptText === 'string' ? promptText : '') : ((nc as any)?.defaultValue ?? '')
    ...
    const { data: insertedWork, error: insErr } = await supabase
      ...
      .single()
    if (insErr) {
      return Response.json({ success: false, message: `保存占位作品失败：${insErr.message}` }, { status: 500 })
    }
    ...
    const { data: genRecord, error: genErr } = await supabase
      ...
      .insert({
        ...
        template_id: workId,
        input_type: 'image',
        input_data: { resolvedValues, promptText, workId },
        source_urls: [],
        generation_params: { request_json, target: 'work', work_id: insertedWork.id, prompt: promptText, work_id_source: workId },
        prompt: promptText,
        output_type: 'image',
        status: 'processing',
        progress: 0,
    ...
    console.error('POST /api/same-style/generate/[workId] error', e)
